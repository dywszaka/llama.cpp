...
def make_heatmap(raw_path: Path, image_path: Path, kind: str, layer: int, n_tokens: int, n_channels: int) -> dict:
    ...
    y_step = 512 if n_channels <= 4096 else 2048
...
def write_html(output: Path, image_dir_name: str, items: list[dict]) -> None:
    ...
    for layer in range(N_LAYERS):
        q = by_layer[layer]["Q"]
        kq = by_layer[layer]["KQ"]
        sections.append(f"""
    <section class="layer-card" id="layer-{layer:02d}">
      <h2>Layer {layer:02d}</h2>
      <div class="pair">
        {image_card(image_dir_name, q)}
        {image_card(image_dir_name, kq)}
      </div>
    </section>""")
...
<head>
  ...
  <title>Q and KQ Heatmaps - One Chunk</title>
  <style>
    :root {{
      color-scheme: light;
      --ink: #271f1a;
...
<body>
  <header>
    <h1>Q and KQ Heatmaps - One Chunk</h1>
    <div class="meta">
      <span>36 layers</span>
      <span>Q: 512 tokens x 4096 channels</span>
      <span>KQ: 512 query tokens x 16384 flattened key/head channels</span>
  ...
  <main>
    <div class="legend">
      <div>Horizontal axis is token position. Vertical axis is channel. KQ channels are flattened as key-token by attention-head. Lighter cells are smaller absolute values; darker cells are larger absolute values.</div>
...
def main() -> None:
    ...
    heatmap_dir = args.output_dir / "q_kq_heatmaps"
    items: list[dict] = []
    for layer in range(N_LAYERS):
        items.append(make_heatmap(
            args.raw_dir / "q_raw_f32" / f"layer_{layer:02d}.f32.bin",
            heatmap_dir / f"q_layer_{layer:02d}.png",
            "Q",
            layer,
            N_TOKENS,
            Q_CHANNELS,
        ...
        items.append(make_heatmap(
            args.raw_dir / "kq_raw_f32" / f"layer_{layer:02d}.f32.bin",
            heatmap_dir / f"kq_layer_{layer:02d}.png",
            "KQ",
            layer,
            N_TOKENS,
            KQ_CHANNELS,
    ...
    html_path = args.output_dir / "q_kq_heatmaps.html"
    manifest_path = args.output_dir / "q_kq_heatmaps_manifest.json"
