...
llama_context * llama_init_from_model(
    ...
    if (params.type_k == GGML_TYPE_NVFP4_8 && !params.offload_kqv) {
        LLAMA_LOG_ERROR("%s: NVFP4_8 K cache requires offload_kqv\n", __func__);
        return nullptr;
    }

    if (params.type_k == GGML_TYPE_FP8_E4M3_E8M0_32 && params.flash_attn) {
        LLAMA_LOG_ERROR("%s: FP8(E4M3+E8M0) K cache does not support flash_attn yet\n", __func__);
        return nullptr;
    }
    ...
    if (params.type_k == GGML_TYPE_FP8_E4M3_E8M0_32 && !params.offload_kqv) {
        LLAMA_LOG_ERROR("%s: FP8(E4M3+E8M0) K cache requires offload_kqv\n", __func__);
        return nullptr;
    }

    if (hybrid_fp8_kcache && !params.offload_kqv) {
        LLAMA_LOG_ERROR("%s: hybrid FP8(E4M3+E8M0) K cache requires offload_kqv\n", __func__);
        return nullptr;
    }

    if ((params.type_v == GGML_TYPE_FP8_E4M3_E8M0_32 ||
         params.type_v == GGML_TYPE_FP8_E4M3_E8M0_16) && !params.flash_attn && !params.offload_kqv) {
