  ...
  if CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}" \
    ...
    LLAMA_NVFP4_KCACHE_OUTLIER_THRESHOLD="${threshold}" \
      "${ROOT_DIR}/build_cuda/bin/llama-perplexity" \
        ...
        --chunks "${CHUNKS}" \
        > "${log_file}" 2>&1; then
    echo "threshold=${threshold} status=ok" | tee "${EXP_DIR}/runs/threshold_${threshold}.status"
  else
    status=$?
    echo "threshold=${threshold} status=failed exit=${status}" | tee "${EXP_DIR}/runs/threshold_${threshold}.status"
  fi
done
