...
- CUDA execution path:
  ...
  - Native implementation is in `ggml/src/ggml-cuda/expt/nvfp4/nvfp4-matmul.cu`.
...
- CUDA NVFP4 V-cache p*v matmul lives in `ggml/src/ggml-cuda/expt/nvfp4/vcache-nvfp4-matmul.cu`.
  - The default global-scale V-cache p*v matmul path builds native 2D slices and lets the native NVFP4 matmul path dynamically quantize F32 P.
  - `ggml_cuda_vcache_nvfp4_matmul_fp4_p_lt` is the V-cache-specific fallback path.
- CUDA fallback path:
