...
void c100_llama_stop(void) __attribute__((weak));
...
bool ggml_backend_c100_is_available(void) {
    return c100_llama_init && c100_llama_start && get_simulator_instance;
}
...
ggml_backend_t ggml_backend_c100_init(void) {
    if (!ggml_backend_c100_is_available()) {
        fprintf(stderr, "[ERROR] C100: runtime API not available\n");
        return NULL;
    }
    ...
    if (!c100_llama_start()) {
        fprintf(stderr, "[ERROR] C100: failed to start simulator runtime\n");
        return NULL;
    }
