...
void quantize_row_fp8_e4m3_e8m0_16_ref(const float * GGML_RESTRICT x, block_fp8_e4m3_e8m0_16 * GGML_RESTRICT y, int64_t k) {
    assert(k % QK_FP8_E4M3_E8M0_16 == 0);

    const int64_t nb = k / QK_FP8_E4M3_E8M0_16;
    for (int64_t ib = 0; ib < nb; ++ib) {
        const float * xb = x + ib * QK_FP8_E4M3_E8M0_16;
        float amax = 0.0f;
...
void dequantize_row_fp8_e4m3_e8m0_16(const block_fp8_e4m3_e8m0_16 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    assert(k % QK_FP8_E4M3_E8M0_16 == 0);
    ...
    for (int64_t ib = 0; ib < nb; ++ib) {
        const float scale = x[ib].e == 0 ? 0.0f : GGML_E8M0_TO_FP32(x[ib].e);
        for (int j = 0; j < QK_FP8_E4M3_E8M0_16; ++j) {
            y[ib * QK_FP8_E4M3_E8M0_16 + j] = GGML_E4M3_TO_FP32(x[ib].qs[j]) * scale;
        }
    }
}
...
size_t quantize_fp8_e4m3_e8m0_16(const float * GGML_RESTRICT src, void * GGML_RESTRICT dst, int64_t nrow, int64_t n_per_row, const float * quant_weights) {
    GGML_UNUSED(quant_weights);
    quantize_row_fp8_e4m3_e8m0_16_ref(src, (block_fp8_e4m3_e8m0_16 *) dst, (int64_t) nrow*n_per_row);
    return nrow * ggml_row_size(GGML_TYPE_FP8_E4M3_E8M0_16, n_per_row);
}
...
bool ggml_validate_row_data(enum ggml_type type, const void * data, size_t nbytes) {
    ...
    switch (type) {
        ...
        case GGML_TYPE_FP8_E4M3_E8M0_16:
            {
                VALIDATE_ROW_DATA_E_E8M0_IMPL(block_fp8_e4m3_e8m0_16, data, nb);
            } break;
