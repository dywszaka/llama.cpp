...
static __global__ void glu_qemu_preprocess_kernel(
    ...
    for (size_t index = (size_t) blockIdx.x * blockDim.x + threadIdx.x;
        ...
        src0_bf16[index] = glu_fp32_f32_to_bf16_rz(
                load_native_value(params.src0, params.src0_type, src0_index));
        src1_bf16[index] = glu_fp32_f32_to_bf16_rz(
                load_native_value(params.src1, params.src1_type, src1_index));
    }
}

static __global__ void glu_qemu_bf16_kernel(
        const uint16_t * src0,
        const uint16_t * src1,
        uint16_t * dst,
        size_t elements) {
    for (size_t index = (size_t) blockIdx.x * blockDim.x + threadIdx.x;
            index < elements;
            index += (size_t) blockDim.x * gridDim.x) {
        dst[index] = glu_fp32_swiglu_bits(src0[index], src1[index]);
    }
}

static __global__ void glu_qemu_output_kernel(
        ggml_cuda_glu_qemu_params params,
    ...
    for (size_t index = (size_t) blockIdx.x * blockDim.x + threadIdx.x;
        ...
        const float value = glu_fp32_bf16_to_f32(input[index]);
...
static int glu_qemu_cuda_blocks(size_t elements) {
    constexpr size_t threads = 256;
    return (int) std::min<size_t>((elements + threads - 1) / threads, 65535);
}

static size_t glu_qemu_elements(const ggml_cuda_glu_qemu_params & params) {
    return (size_t) params.ne[0] * (size_t) params.ne[1] *
            (size_t) params.ne[2] * (size_t) params.ne[3];
}

void ggml_cuda_glu_qemu_cuda_preprocess(
        const ggml_cuda_glu_qemu_params & params,
        uint16_t * src0_bf16,
        uint16_t * src1_bf16,
        cudaStream_t stream) {
    const size_t elements = glu_qemu_elements(params);
    if (elements == 0) {
        return;
    }
    constexpr int threads = 256;
    glu_qemu_preprocess_kernel<<<glu_qemu_cuda_blocks(elements), threads, 0, stream>>>(
            params, src0_bf16, src1_bf16, elements);
    CUDA_CHECK(cudaGetLastError());
}

void ggml_cuda_glu_qemu_cuda_run_bf16(
        const ggml_cuda_glu_qemu_params & params,
    ...
    constexpr int threads = 256;
    glu_qemu_bf16_kernel<<<glu_qemu_cuda_blocks(elements), threads, 0, stream>>>(
            src0_bf16, src1_bf16, dst_bf16, elements);
    CUDA_CHECK(cudaGetLastError());
}

void ggml_cuda_glu_qemu_cuda_output(
        const ggml_cuda_glu_qemu_params & params,
    ...
    constexpr int threads = 256;
    glu_qemu_output_kernel<<<glu_qemu_cuda_blocks(elements), threads, 0, stream>>>(
