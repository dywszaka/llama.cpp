...
static __device__ __forceinline__ uint16_t mul_bf16_from_f32_rz(float value) {
    return (uint16_t) (__float_as_uint(value) >> 16);
}

static __device__ __forceinline__ uint16_t mul_bf16_from_f32_rne(float value) {
...
static __device__ __forceinline__ float mul_bf16_to_f32(uint16_t value) {
...
static __global__ void mul_qemu_preprocess_kernel(
    ...
    for (size_t index = (size_t) blockIdx.x * blockDim.x + threadIdx.x;
        ...
        src0_bf16[index] = mul_bf16_from_f32_rz(
                load_native_value(params.src0, params.src0_type, src0_index));
        src1_bf16[index] = mul_bf16_from_f32_rz(
                load_native_value(params.src1, params.src1_type, src1_index));
    }
}

static __global__ void mul_qemu_bf16_kernel(
        const uint16_t * src0,
        const uint16_t * src1,
        uint16_t * dst,
        size_t elements) {
    for (size_t index = (size_t) blockIdx.x * blockDim.x + threadIdx.x;
        ...
        const float product = __fmul_rn(
                mul_bf16_to_f32(src0[index]),
                mul_bf16_to_f32(src1[index]));
        dst[index] = mul_bf16_from_f32_rne(product);
    }
}

static __global__ void mul_qemu_output_kernel(
        ggml_cuda_mul_qemu_params params,
    ...
    for (size_t index = (size_t) blockIdx.x * blockDim.x + threadIdx.x;
        ...
        const float value = mul_bf16_to_f32(input[index]);
...
static int mul_qemu_cuda_blocks(size_t elements) {
    constexpr size_t threads = 256;
    return (int) std::min<size_t>((elements + threads - 1) / threads, 65535);
}

static size_t mul_qemu_elements(const ggml_cuda_mul_qemu_params & params) {
    return (size_t) params.ne[0] * (size_t) params.ne[1] *
            (size_t) params.ne[2] * (size_t) params.ne[3];
}

void ggml_cuda_mul_qemu_cuda_preprocess(
        const ggml_cuda_mul_qemu_params & params,
        uint16_t * src0_bf16,
        uint16_t * src1_bf16,
        cudaStream_t stream) {
    const size_t elements = mul_qemu_elements(params);
    if (elements == 0) {
        return;
    }
    constexpr int threads = 256;
    mul_qemu_preprocess_kernel<<<mul_qemu_cuda_blocks(elements), threads, 0, stream>>>(
            params, src0_bf16, src1_bf16, elements);
    CUDA_CHECK(cudaGetLastError());
}

void ggml_cuda_mul_qemu_cuda_run_bf16(
        const ggml_cuda_mul_qemu_params & params,
    ...
    constexpr int threads = 256;
    mul_qemu_bf16_kernel<<<mul_qemu_cuda_blocks(elements), threads, 0, stream>>>(
            src0_bf16, src1_bf16, dst_bf16, elements);
    CUDA_CHECK(cudaGetLastError());
}

void ggml_cuda_mul_qemu_cuda_output(
        const ggml_cuda_mul_qemu_params & params,
    ...
    constexpr int threads = 256;
    mul_qemu_output_kernel<<<mul_qemu_cuda_blocks(elements), threads, 0, stream>>>(
