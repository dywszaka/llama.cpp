...
static __global__ void k_apply_outlier_correction(
    ...
    const int64_t n = min((int64_t) count, max_outliers);
    float corr = 0.0f;
    for (int64_t i = 0; i < n; ++i) {
        const int64_t global_dim = indices[kv_pos * max_outliers + i];
        ...
        corr += values[kv_pos * max_outliers + i] * qv;
    }
...
void ggml_cuda_nvfp4_kcache_outlier_extract(
    ...
    k_reset_outlier_rows<<<reset_grid, block_size, 0, stream>>>(
            dst_rows, counts, ne01, sidecar_rows, dst_rows_stride);
    ...
    if (ggml_cuda_nvfp4_kcache_outlier_log_enabled() && can_copy_counts_for_log(stream)) {
        ...
        GGML_LOG_INFO(
                "%s: rows=%lld threshold=%g stored_max=%lld total_outliers=%lld max_row_outliers=%d overflow_rows=%lld\n",
                __func__,
                (long long) ne01,
                (double) threshold,
...
void ggml_cuda_nvfp4_kcache_outlier_apply_correction(
    ...
    k_apply_outlier_correction<<<grid_size, block_size, 0, stream>>>(
            counts, indices, values, q, kq, k_scale,
            head_dim, kv_len, q_len, q_heads, kv_heads, q_head, max_outliers,
