#include "add-qemu.cuh"
#include "add-qemu-cuda.cuh"
#include "add-qemu-protocol.h"
...
#if defined(GGML_CUDA_ADD_QEMU)
#include <zmq.h>
#endif

static const char * GGML_CUDA_ADD_QEMU_MODE_ENV = "GGML_CUDA_ADD_QEMU_MODE";
static const char * GGML_CUDA_ADD_QEMU_ENDPOINT_ENV = "GGML_CUDA_ADD_QEMU_ENDPOINT";
static const char * GGML_CUDA_ADD_QEMU_TIMEOUT_ENV = "GGML_CUDA_ADD_QEMU_TIMEOUT_MS";
static const char * GGML_CUDA_ADD_QEMU_ARTIFACT_ENV = "GGML_CUDA_ADD_QEMU_ARTIFACT";
static const char * GGML_CUDA_ADD_QEMU_MISMATCH_LOG_ENV = "GGML_CUDA_ADD_QEMU_MISMATCH_LOG";
static const char * GGML_CUDA_ADD_QEMU_TIMING_ENV = "GGML_CUDA_ADD_QEMU_TIMING";
static const char * GGML_CUDA_ADD_QEMU_DEFAULT_ENDPOINT = "tcp://127.0.0.1:15586";
static const char * GGML_CUDA_ADD_QEMU_DEFAULT_ARTIFACT =
        "experiments/add-qemu-compare.jsonl";
static const char * GGML_CUDA_ADD_QEMU_DEFAULT_MISMATCH_LOG =
        "experiments/add-qemu-cuda-mismatch.jsonl";

static const char * ggml_cuda_add_qemu_mode_name(ggml_cuda_add_qemu_mode mode) {
    switch (mode) {
        case GGML_CUDA_ADD_QEMU_MODE_CUDA:      return "cuda";
        case GGML_CUDA_ADD_QEMU_MODE_QEMU:      return "qemu";
        case GGML_CUDA_ADD_QEMU_MODE_QEMU_CUDA: return "qemu_cuda";
        case GGML_CUDA_ADD_QEMU_MODE_COMPARE:   return "compare";
    }
    return "cuda";
}

static ggml_cuda_add_qemu_mode parse_mode(const char * value) {
    if (value == nullptr || value[0] == '\0' || std::strcmp(value, "cuda") == 0) {
        return GGML_CUDA_ADD_QEMU_MODE_CUDA;
    }
    if (std::strcmp(value, "qemu") == 0) {
        return GGML_CUDA_ADD_QEMU_MODE_QEMU;
    }
    if (std::strcmp(value, "qemu_cuda") == 0) {
        return GGML_CUDA_ADD_QEMU_MODE_QEMU_CUDA;
    }
    if (std::strcmp(value, "compare") == 0 ||
            std::strcmp(value, "compare_cuda") == 0 ||
            std::strcmp(value, "compare_qemu") == 0) {
        return GGML_CUDA_ADD_QEMU_MODE_COMPARE;
    }
    GGML_LOG_WARN("%s: unknown %s=%s; using cuda\n",
            __func__, GGML_CUDA_ADD_QEMU_MODE_ENV, value);
    return GGML_CUDA_ADD_QEMU_MODE_CUDA;
}

ggml_cuda_add_qemu_mode ggml_cuda_add_qemu_get_mode() {
    static const ggml_cuda_add_qemu_mode mode =
            parse_mode(std::getenv(GGML_CUDA_ADD_QEMU_MODE_ENV));
    return mode;
}

bool ggml_cuda_add_qemu_enabled() {
    return ggml_cuda_add_qemu_get_mode() != GGML_CUDA_ADD_QEMU_MODE_CUDA;
}

static std::string rpc_endpoint() {
    const char * value = std::getenv(GGML_CUDA_ADD_QEMU_ENDPOINT_ENV);
    return value != nullptr && value[0] != '\0' ? value :
            GGML_CUDA_ADD_QEMU_DEFAULT_ENDPOINT;
}

static int rpc_timeout_ms() {
    const char * value = std::getenv(GGML_CUDA_ADD_QEMU_TIMEOUT_ENV);
...
static std::string artifact_path() {
    const char * value = std::getenv(GGML_CUDA_ADD_QEMU_ARTIFACT_ENV);
    return value != nullptr && value[0] != '\0' ? value :
            GGML_CUDA_ADD_QEMU_DEFAULT_ARTIFACT;
}

static std::string mismatch_log_path() {
    const char * value = std::getenv(GGML_CUDA_ADD_QEMU_MISMATCH_LOG_ENV);
    return value != nullptr && value[0] != '\0' ? value :
            GGML_CUDA_ADD_QEMU_DEFAULT_MISMATCH_LOG;
}

static bool timing_enabled() {
    static const bool enabled = [] {
        const char * value = std::getenv(GGML_CUDA_ADD_QEMU_TIMING_ENV);
...
static void log_mode_once(ggml_cuda_add_qemu_mode mode) {
    static std::atomic<bool> logged(false);
    if (mode == GGML_CUDA_ADD_QEMU_MODE_CUDA || logged.exchange(true)) {
        return;
    }
    if (mode == GGML_CUDA_ADD_QEMU_MODE_QEMU_CUDA) {
        GGML_LOG_INFO(
                "%s: %s=qemu_cuda enabled; FP32-compute/BF16-I/O ADD stays on the CUDA device, "
                "canonical_input=RZ, ZMQ/D2H/H2D are not used, timing=%s\n",
                __func__, GGML_CUDA_ADD_QEMU_MODE_ENV,
                timing_enabled() ? "on" : "off");
        return;
    }
    const std::string endpoint = rpc_endpoint();
    if (mode == GGML_CUDA_ADD_QEMU_MODE_COMPARE) {
        GGML_LOG_INFO(
                "%s: %s=compare enabled; downstream=llama CUDA, endpoint=%s, "
                "canonical_input=RZ, timing=%s, artifact=%s, mismatch_log=%s\n",
                __func__, GGML_CUDA_ADD_QEMU_MODE_ENV, endpoint.c_str(),
                timing_enabled() ? "on" : "off", artifact_path().c_str(),
                mismatch_log_path().c_str());
        return;
    }
    GGML_LOG_INFO(
            "%s: %s=qemu enabled; FP32-compute/BF16-I/O RVV ADD endpoint=%s, "
            "canonical_input=RZ, timing=%s\n",
            __func__, GGML_CUDA_ADD_QEMU_MODE_ENV, endpoint.c_str(),
            timing_enabled() ? "on" : "off");
}

static size_t add_elements(const ggml_cuda_add_qemu_params & params) {
...
static add_compare_metrics compare_outputs(
...
static void write_comparison_artifact(
        ...
        const add_qemu_result & qemu,
        const add_compare_metrics & metrics,
        float llama_cuda_ms,
        const add_qemu_cuda_timing & qemu_cuda_timing) {
...
static void write_mismatch_log(
    ...
    output << "}\n";
    GGML_LOG_ERROR(
            "QEMU_CUDA_ADD_BIT_MISMATCH request=%llu dst=%s mismatches=%zu "
            "first=%zu log=%s\n",
...
static void log_qemu_timing(
        const ggml_tensor * dst_tensor,
        const add_qemu_result & result,
        size_t elements,
        uint64_t return_copy_ns,
        ggml_cuda_add_qemu_mode mode) {
    if (!timing_enabled()) {
        return;
    }
    const char * tensor_name = dst_tensor->name[0] != '\0' ?
...
static void log_qemu_cuda_timing(
        const ggml_tensor * dst_tensor,
        const ggml_cuda_add_qemu_params & params,
        const add_qemu_cuda_timing & timing,
        ggml_cuda_add_qemu_mode mode) {
    if (!timing_enabled()) {
        return;
    }
    ...
    const char * tensor_name = dst_tensor->name[0] != '\0' ?
            dst_tensor->name : "(unnamed)";
    GGML_LOG_INFO(
            "QEMU_CUDA_ADD_TIMING mode=%s dst=%s shape=%lldx%lldx%lldx%lld "
            "preprocess_ms=%.3f operator_ms=%.3f output_ms=%.3f total_ms=%.3f "
            "calls=%llu average_total_ms=%.3f\n",
            ggml_cuda_add_qemu_mode_name(mode), tensor_name,
...
static add_qemu_result call_qemu_rpc(
        const ggml_cuda_add_qemu_params & params,
    ...
    add_rpc_request_v1 request = {};
    request.magic = ADD_RPC_MAGIC;
    request.version = ADD_RPC_VERSION;
    request.header_bytes = sizeof(request);
    request.request_id = next_request_id.fetch_add(1);
    request.flags = ADD_RPC_REQUEST_CANONICAL_DENSE;
    request.src0_type = ADD_RPC_DTYPE_BF16;
    request.src1_type = ADD_RPC_DTYPE_BF16;
    request.dst_type = ADD_RPC_DTYPE_BF16;
    ...
    if (!more || response_frame.size() != sizeof(add_rpc_response_v1)) {
        GGML_ABORT("%s: invalid QEMU RPC response header\n", __func__);
    }
    add_rpc_response_v1 response = {};
...
static add_qemu_result run_qemu_rpc(
        const ggml_cuda_add_qemu_params & params,
    ...
    GGML_UNUSED(stream);
    GGML_ABORT("%s: llama.cpp was built without GGML_CUDA_ADD_QEMU=ON\n", __func__);
#else
    ...
    add_qemu_result result = call_qemu_rpc(
...
static void run_qemu_cuda_only(
    ...
    if (timing_enabled()) {
        ...
        log_qemu_cuda_timing(
                dst_tensor, params, timing, GGML_CUDA_ADD_QEMU_MODE_QEMU_CUDA);
...
static void run_qemu_only(
    ...
    ggml_cuda_add_qemu_cuda_preprocess(
            params, src0_bf16.get(), src1_bf16.get(), stream);
    add_qemu_result qemu = run_qemu_rpc(
    ...
    log_qemu_timing(
            dst_tensor, qemu, elements, return_copy_ns, GGML_CUDA_ADD_QEMU_MODE_QEMU);
}
...
static void run_compare(
        ...
        const ggml_tensor * dst_tensor,
        const ggml_cuda_add_qemu_params & params,
        ggml_cuda_add_launch_fn cuda_launch,
        size_t elements) {
    ...
    ggml_cuda_add_qemu_params cuda_params = params;
    cuda_params.dst = cuda_dst.get();
    ggml_cuda_add_qemu_params qemu_cuda_params = params;
    ...
    ggml_cuda_add_qemu_cuda_output(
    ...
    const add_compare_metrics metrics = compare_outputs(
    ...
    if (timing_enabled()) {
        const char * tensor_name = dst_tensor->name[0] != '\0' ?
                dst_tensor->name : "(unnamed)";
        GGML_LOG_INFO(
                "LLAMA_CUDA_ADD_TIMING mode=compare dst=%s elements=%zu total_ms=%.3f\n",
                tensor_name, elements, llama_cuda_ms);
    }
    log_qemu_timing(
            dst_tensor, qemu_result, elements, 0, GGML_CUDA_ADD_QEMU_MODE_COMPARE);
    log_qemu_cuda_timing(
            dst_tensor, params, qemu_cuda_timing, GGML_CUDA_ADD_QEMU_MODE_COMPARE);
...
void ggml_cuda_add_qemu_run(
        ggml_backend_cuda_context & ctx,
        const ggml_tensor * dst_tensor,
        const ggml_cuda_add_qemu_params & params,
        ggml_cuda_add_launch_fn cuda_launch) {
    const ggml_cuda_add_qemu_mode mode = ggml_cuda_add_qemu_get_mode();
    log_mode_once(mode);
    const size_t elements = add_elements(params);
    GGML_ASSERT(elements == (size_t) ggml_nelements(dst_tensor));

    switch (mode) {
        case GGML_CUDA_ADD_QEMU_MODE_QEMU:
            run_qemu_only(ctx, dst_tensor, params, elements);
            return;
        case GGML_CUDA_ADD_QEMU_MODE_QEMU_CUDA:
            run_qemu_cuda_only(ctx, dst_tensor, params, elements);
            return;
        case GGML_CUDA_ADD_QEMU_MODE_COMPARE:
            run_compare(ctx, dst_tensor, params, cuda_launch, elements);
            return;
        case GGML_CUDA_ADD_QEMU_MODE_CUDA:
            GGML_ABORT("%s: CUDA-only mode must use the original ADD dispatch\n", __func__);
    }
}
