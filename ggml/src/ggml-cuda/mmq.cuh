...
template <int mmq_y, bool need_check> static __device__ __forceinline__ void load_tiles_nvfp4(
    ...
    constexpr int threads_per_row = MMQ_ITER_K / (4 * QR_NVFP4);
    constexpr int nrows = warp_size / threads_per_row;
    const int txi = warp_size > threads_per_row ? threadIdx.x % threads_per_row : threadIdx.x;
    const int kbx  = txi / QI_NVFP4;
    const int kqsx = txi % QI_NVFP4;
...
#pragma unroll
    for (int i0 = 0; i0 < mmq_y; i0 += nrows*nwarps) {
        ...
        const uint32_t vx = (uint32_t) v.x;
        const uint32_t vy = (uint32_t) v.y;

        const uint32_t vx0 =  vx        & 0xFFu;
        const uint32_t vx1 = (vx >>  8) & 0xFFu;
        const uint32_t vx2 = (vx >> 16) & 0xFFu;
        const uint32_t vx3 = (vx >> 24) & 0xFFu;
        const uint32_t vy0 =  vy        & 0xFFu;
        const uint32_t vy1 = (vy >>  8) & 0xFFu;
        const uint32_t vy2 = (vy >> 16) & 0xFFu;
        const uint32_t vy3 = (vy >> 24) & 0xFFu;

        const int out0 = (int) (vx0 | (vy0 << 8) | (vx1 << 16) | (vy1 << 24));
        const int out1 = (int) (vx2 | (vy2 << 8) | (vx3 << 16) | (vy3 << 24));

        const int k0 = kbx * (2 * QI_NVFP4) + kqsx*2;

#if defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE)
        x_qs[i*MMQ_MMA_TILE_X_K_Q8_1 + k0 + 0] = out0;
        x_qs[i*MMQ_MMA_TILE_X_K_Q8_1 + k0 + 1] = out1;
#else
        x_qs[i*(2*MMQ_TILE_NE_K + 1) + k0 + 0] = out0;
        x_qs[i*(2*MMQ_TILE_NE_K + 1) + k0 + 1] = out1;
#endif // defined(AMD_MFMA_AVAILABLE) || defined(TURING_MMA_AVAILABLE)
    }

    constexpr int blocks_per_tile_x_row = MMQ_TILE_NE_K / QI_NVFP4;
    constexpr int rows_per_warp = warp_size / blocks_per_tile_x_row;
    const int kbxd = threadIdx.x % blocks_per_tile_x_row;
