...
grep -q "NVFP4 K-cache compact outlier sidecar enabled by default: threshold=16 layer_capacities=36" "${TMP_OUTPUT}"
grep -q "K (nvfp4+fp8_e4m3_e8m0_32)" "${TMP_OUTPUT}"
if grep -q "LLAMA_NVFP4_KCACHE_OUTLIER" "${TMP_OUTPUT}"; then
    echo "unexpected removed NVFP4 outlier environment switch in output" >&2
    exit 1
fi
