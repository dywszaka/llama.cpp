...
if ! command -v nvidia-smi >/dev/null 2>&1 || ! nvidia-smi -L >/dev/null 2>&1; then
    echo "test-kcache-outlier-hybrid-b-switch-smoke: SKIP (CUDA device unavailable)"
    exit 0
fi
...
if grep -q "CPU KV buffer size" "${TMP_OUTPUT}"; then
    echo "test-kcache-outlier-hybrid-b-switch-smoke: SKIP (KV cache was not allocated on CUDA)"
    exit 0
fi
...
if grep -q "NVFP4 K-cache compact outlier sidecar" "${TMP_OUTPUT}"; then
    echo "B switch alone should not enable NVFP4 K-cache outlier sidecar" >&2
    exit 1
fi
